...
from fedml_api.distributed.decentralized_framework.message_define import MyMessage
...
class DecentralizedWorkerManager(ClientManager):
    ...
    def handle_msg_from_neighbor(self, msg_params):
        ...
        logging.info(f"handle_msg_from_neighbor. sender_id = {sender_id}")
        self.trainer.add_result(sender_id, training_interation_result)
        if self.trainer.check_whether_all_receive():
            logging.info(f">>>>>>>>>>>>>>>WORKER {self.worker_index:d}, ROUND {self.round_idx:d} finished!<<<<<<<<")
            self.round_idx += 1
    ...
    def send_result_to_neighbors(self, receive_id, client_params1):
        logging.info(f"send_result_to_neighbors. receive_id = {receive_id}")
