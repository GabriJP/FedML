...
class StackOverflowNWPDataLoader(DataLoader):
    def get_dataloader(self, client_idx=None):
        def _tokenizer(x):
            return utils.tokenizer(x, self.data_dir)

        if client_idx is None:

            train_dl = data.DataLoader(data.ConcatDataset(
                StackOverflowDataset(os.path.join(self.data_dir, DEFAULT_TRAIN_FILE), client_idx, _tokenizer)
                for client_idx in range(DEFAULT_TRAIN_CLIENTS_NUM)), batch_size=self.train_bs, shuffle=True)

            test_dl = data.DataLoader(data.ConcatDataset(
                StackOverflowDataset(os.path.join(self.data_dir, DEFAULT_TEST_FILE), client_idx, "test", _tokenizer)
                for client_idx in range(DEFAULT_TEST_CLIENTS_NUM)), batch_size=self.test_bs, shuffle=True)
            return train_dl, test_dl

        else:
            train_ds = StackOverflowDataset(
                os.path.join(self.data_dir, DEFAULT_TRAIN_FILE), client_idx, "train", _tokenizer)
            ...
            if client_idx >= DEFAULT_TEST_CLIENTS_NUM:
                ...
                test_ds = StackOverflowDataset(
                    os.path.join(self.data_dir, DEFAULT_TEST_FILE), client_idx, "test", _tokenizer)
    ...
    def load_partition_data_distributed_federated(self, process_id):
        ...
        VOCAB_LEN = len(utils.get_word_dict(self.data_dir)) + 1
        return DistributedFederatedDataset(
            client_num=DEFAULT_TRAIN_CLIENTS_NUM, train_data_num=train_data_num, train_data_global=train_data_global,
            test_data_global=test_data_global, local_data_num=local_data_num, train_data_local=train_data_local,
            test_data_local=test_data_local, output_len=VOCAB_LEN)
    ...
    def load_partition_data_federated(self, batch_size=DEFAULT_BATCH_SIZE):
        ...
        return FederatedDataset(
            ...
            test_data_local_dict=test_data_local_dict, vocab_len=VOCAB_LEN)


if __name__ == "__main__":
    # load_partition_data_federated_stackoverflow(None, None, 100, 128)
    dl = StackOverflowNWPDataLoader(None, 128, 128)
